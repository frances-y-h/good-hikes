...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        // For geting the numbers of review and average rating
        ...
        const hikeDescription = [
            `Try this ${
                hike.length
            }-mile ${hike.RouteType.name.toLowerCase()}trail. It's considered to be ${hike.Difficulty.level.toLowerCase()} in difficulty. ${
                hike.name
            } is located in ${hike.CityPark.name}, ${
                hike.State.state
            }. Breathtaking views with ${
                hike.elevationChange
            } ft of elevation change. This is a popular trail for hiking, so expect to see fellow hikers out there with you.`,
            `${hike.State.state} has got some beautiful trails, and ${
                hike.name
            } will not disappoint. With ${
                hike.elevationChange
            } ft of elevation change, this ${hike.Difficulty.level.toLowerCase()} trail will get your blood flowing. Expect to see a few people, but not too many, for complete solitude, explore on a weekend. Located inside ${
                hike.CityPark.name
            } this beautiful ${hike.RouteType.name.toLowerCase()} jaunt is only ${
                hike.length
            } miles long.`,
            `What a complete joy ${
                hike.name
            } is. Not only is it in the wonderful state of ${
                hike.State.state
            }, but it's also considered ${hike.Difficulty.level.toLowerCase()} by most hikers. Take a ${
                hike.length
            } stroll on this ${hike.RouteType.name.toLowerCase()} trail in ${
                hike.CityPark.name
            }. At only ${
                hike.elevationChange
            } ft of elevation change, you can soak in the sun and breathe in some fresh air. Head out early to get all you can out of this trail.`,
            `The best trail to go to that is surprisingly not busy on the weekend? ${
                hike.name
            }. I know, I'm shocked too. Rated at ${hike.Difficulty.level.toLowerCase()}, this hike nestled in ${
                hike.State.state
            } gives you plenty to see. Located in ${
                hike.CityPark.name
            }, it's a measly ${
                hike.length
            } miles long, elevation change is only ${
                hike.elevationChange
            } ft, and it's simply best ${hike.RouteType.name.toLowerCase()} trail in the area! Just get out there already!`,
            `${hike.CityPark.name} has plenty to offer, but ${
                hike.name
            } should be at the top of your list. Topping out at ${
                hike.elevationChange
            } ft of elevation change, it's your simple ${hike.RouteType.name.toLowerCase()} trail, but the ${
                hike.State.state
            } air makes up for the ${hike.Difficulty.level.toLowerCase()} hike you're embarking on. Plenty of time to see everything and stop and smell the roses on this ${
                hike.length
            } mile hike, and still get home before it's dark.`,
            `${
                hike.RouteType.name
            } trails get you up in the morning? What about a little lift of, I don't know, say, ${
                hike.elevationChange
            } ft? Come out to ${
                hike.name
            } for your morning dose of nature. It's a popular trail because of all to see on it's ${
                hike.length
            } mile stroll in ${hike.State.state}'s ${
                hike.CityPark.name
            }. Come early to get parking, so you don't have to add too much to the hike. But if you can't find parking, don't worry, it's a very ${hike.Difficulty.level.toLowerCase()} hike. You got this.`,
            ...
            `${hike.State.state}, can you do no wrong? You gave us ${
                hike.name
            }, and we cannot thank you enough. What did we deserve to enjoy the majesty you bring the world. Looking for something ${hike.Difficulty.level.toLowerCase()} to do this weekend? Check out this ${hike.RouteType.name.toLowerCase()} hike, heads up though it's got ${
                hike.elevationChange
            } ft of elevation change, nothing to laugh at on this ${
                hike.length
            } mile length trail. Plenty to enjoy, I can't think of anyone who doesn't love going to ${
                hike.CityPark.name
            }.`,
            `${hike.RouteType.name}, the tried and true trail. This ${
                hike.length
            } trail inside the wonderful state of ${
                hike.State.state
            } won't disappoint. ${
                hike.elevationChange
            } ft difference in the distance from our earth's core on ${
                hike.name
            }. There's always something to do in ${
                hike.CityPark.name
            }, so enjoy a long day outside with this ${hike.Difficulty.level.toLowerCase()} trail.`,
        ];
